...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Option 2: Use browser wallet (MetaMask etc)
  ...
  const connectBrowserWallet = async () => {
    ...
    try {
      ...
      const wrappedFetch = wrapFetchWithPayment(fetch, client as any);
      setFetchWithPayment(() => wrappedFetch);
      setError(null);
    } catch (err: any) {
  ...
  const handleGetFortune = async () => {
    ...
    try {
      console.log("Making request with fetchWithPayment...");
      const res = await fetchWithPayment("/api/fortune");
      console.log("Response status:", res.status);

      // x402-fetch should handle 402 automatically, but if it returns 402,
      // it means payment failed or wasn't processed
      if (!res.ok && res.status !== 402) {
        throw new Error(`Request failed: ${res.status} ${res.statusText}`);
      }

      if (res.status === 402) {
        // Payment wasn't handled - show specific error
          ...
          {!walletAddress ? (
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                type="button"
              >
                {loading ? "Processing..." : "Open Fortune Cookie (0.0001 ETH)"}
              </button>
            </div>
          )}
        </div>
